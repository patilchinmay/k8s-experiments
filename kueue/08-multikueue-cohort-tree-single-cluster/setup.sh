#!/usr/bin/env bash
# setup.sh
# Creates two Kind clusters (kueue-manager + kueue-worker-1) and installs
# cert-manager + Kueue + all job-framework CRDs on all clusters for the
# MultiKueue + Cohort Tree + Single Cluster experiment.
...
# values.yaml so that Kueue can watch these CRDs on all clusters.
...
# has its CRDs present on all clusters; otherwise the Watch call fails with
# "no matches for kind" and the MultiKueueCluster goes Active=False.
# ---------------------------------------------------------------------------
install_crds() {
  local context="$1"
  echo ""
...
# ---------------------------------------------------------------------------
# 3. Install job-framework CRDs on ALL clusters
#    MultiKueue mirrors JobSet objects to workers; the JobSet CRD must exist
#    on all clusters or Watch calls fail with "no matches for kind".
# ---------------------------------------------------------------------------
install_crds "kind-kueue-manager"
install_crds "kind-kueue-worker-1"

# ---------------------------------------------------------------------------
# 4. Install Kueue on all clusters
#    - Manager: values.yaml has MultiKueue feature gate enabled
#    - Worker:  same values.yaml; MultiKueue feature gate is harmless on workers
#               (they won't create MultiKueue objects there)
# ---------------------------------------------------------------------------
install_kueue "kind-kueue-manager"  "${SCRIPT_DIR}/values.yaml"
install_kueue "kind-kueue-worker-1" "${SCRIPT_DIR}/values.yaml"

# ---------------------------------------------------------------------------
# 5. Extract worker kubeconfig and store as Secret on the manager.
...
echo "     kueue-worker-1: 2x general compute nodes (team-ml + team-platform)"
echo ""
echo "   Worker kubeconfig Secret created in kueue-system on manager:"
