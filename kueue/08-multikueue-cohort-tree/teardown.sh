#!/usr/bin/env bash
# teardown.sh
# Cleans up all resources created during the MultiKueue + Cohort Tree experiment.
...
# ---------------------------------------------------------------------------
for TEAM in team-ml team-data team-platform; do
  ...
  kubectl delete localqueue --all -n "${TEAM}" --ignore-not-found --context "${MANAGER_CTX}"
...
kubectl delete cohort --all --context "${MANAGER_CTX}" --ignore-not-found
...
for CTX in "${WORKER1_CTX}" "${WORKER2_CTX}"; do
  ...
  for TEAM in team-ml team-data team-platform; do
    ...
    kubectl delete localqueue --all -n "${TEAM}" --ignore-not-found --context "${CTX}"
